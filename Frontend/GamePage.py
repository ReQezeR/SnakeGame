...
class GamePage(tk.Frame):
    def setTheme(self, bg):
        ...
        self.snakeLabel = ImageTk.PhotoImage(Image.open("Images/GamePage/snakeMenuLabel.png"))
        self.newGameButtonImage = ImageTk.PhotoImage(Image.open("Images/GamePage/newGameButtonImage.png"))
        self.playButtonImage = ImageTk.PhotoImage(Image.open("Images/GamePage/playButtonImage.png"))
        self.pauseButtonImage = ImageTk.PhotoImage(Image.open("Images/GamePage/pauseButtonImage.png"))
        ...
        self.appleImage = ImageTk.PhotoImage(Image.open("Images/GamePage/apple.png"))
        self.bombImage = ImageTk.PhotoImage(Image.open("Images/GamePage/bomb.png"))
        self.gameOverImage =  ImageTk.PhotoImage(Image.open("Images/GamePage/gameOverImage.png"))

    def createMenu(self, parent):
        menuFrame = tk.Frame(parent, borderwidth=20, background="#FFFFFF")
        tk.Label(menuFrame, text='Database', image=self.snakeLabel, font=self.button_font, bd=10, bg="#F0F0F0").pack(fill=tk.BOTH, expand=1)
        self.createSpace(menuFrame, 30).pack(fill=tk.BOTH, expand=1)
        self.customButton(menuFrame, "New Game", self.newGameButtonImage, lambda: self.new_game(20), x=self.x / 4, xmargin=10, bgcolor="#666666", fontcolor="#8BB0F9").pack(expand=False)
        self.customButton(menuFrame, "Play/Pause", self.playButtonImage, lambda: self.toggle_game_state(), x=self.x / 2, xmargin=10, bgcolor="#666666", fontcolor="#8BB0F9", toggle=1).pack(expand=False)
        self.createSpace(menuFrame, 100).pack(fill=tk.BOTH, expand=1)
        self.customButton(menuFrame, "Return", self.returnButtonImage, lambda: self.returnFunction(), x=self.x / 2, xmargin=10, bgcolor="#666666", fontcolor="#8BB0F9").pack(expand=0)
        self.createSpace(menuFrame, 30).pack(fill=tk.BOTH, expand=1)
        return menuFrame
    ...
    def customButton(self, parent, text, img, comd, x=10, y=1, xmargin=0, ymargin=0, bgcolor="#FFA900", fontcolor="#FFFFFF", toggle=0):
        ...
        temp_label_2 = tk.Label(buttonFrame, width=int(x / 7), height=int(ymargin / 2), bg=self.white).pack()  # add space
        return buttonFrame
    ...
    def _move(self):
        while True:
            sleep(0.2)
            if self.started == False  and self.pause == False:
                self.started = True
            elif self.started == True and self.game_engine.game_over == False and self.pause == False:
                lock = threading.Lock()
                lock.acquire()
                ...
                self.game_canvas.after(10)
                lock.release()
                DbProvider('D:\\Studia\\PROGRAMOWANIE\\Snake\\Backend\\db.sqlite').game_log.insert_into_table(result)
                break;
